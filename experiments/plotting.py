...
def read_and_plot_sil_samples(problem, file, output_dir):
    ...
    title = '{} - {}: Silhouette Samples (KMeans)'.format(ds_readable_name, problem['name'])
    title2 = '{} - {}: Silhouette Samples (GMM)'.format(ds_readable_name, problem['name'])
    df = pd.read_csv(file)
    cluster_sizes = list(set(df['k']))
    for k in cluster_sizes:
        logger.info(" - Processing k={}".format(k))
        p = plot_sil_samples(title, df, k, 'Kmeans')
        p = watermark(p)
        p.savefig(
            '{}/{}/{}_sil_samples_{}.png'.format(output_dir, problem['name'], ds_name, k),
            format='png', bbox_inches='tight', dpi=150)
        p2 = plot_sil_samples(title2, df, k, 'GMM')
        p2 = watermark(p)
        p2.savefig(
            '{}/{}/{}_gmm_sil_samples_{}.png'.format(output_dir, problem['name'], ds_name, k),
            format='png', bbox_inches='tight', dpi=150)
