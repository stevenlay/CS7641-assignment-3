...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset1', action='store_true',
                        help='Run only data set 1')
    parser.add_argument('--dataset2', action='store_true',
                        help='Run only data set 2')
    parser.add_argument('--benchmark', action='store_true',
                        help='Run the benchmark experiments')
    parser.add_argument('--ica', action='store_true',
                        help='Run the ICA experiments')
    parser.add_argument('--pca', action='store_true',
                        help='Run the PCA experiments')
    parser.add_argument('--lda', action='store_true',
                        help='Run the LDA experiments')
    parser.add_argument('--svd', action='store_true',
                        help='Run the SVD experiments')
    parser.add_argument('--rf', action='store_true',
                        help='Run the RF experiments')
    parser.add_argument('--rp', action='store_true',
                        help='Run the RP experiments')
    ...
    if args.dataset1 and args.dataset2:
        logger.error(
            "Can only specify one of '--dataset1' or '--dataset2', not both")
        parser.print_help()
        sys.exit(1)
    ...
    dataset1_details = {
        ...
        'best_nn_params': {'NN__activation': ['relu'], 'NN__alpha': [0.316227766],
                           'NN__hidden_layer_sizes': [(16, 16, 16)], 'NN__learning_rate_init': [0.064]}
    }
    dataset2_details = {
        'data': loader.Spambase(verbose=verbose, seed=seed),
        'name': 'spambase',
        'readable_name': 'Spambase',
        'best_nn_params': {'NN__activation': ['relu'], 'NN__alpha': [0.031622777],
                           'NN__hidden_layer_sizes': [(114, 114, 114)], 'NN__learning_rate_init': [0.008]}
    }
    if args.dataset1:
